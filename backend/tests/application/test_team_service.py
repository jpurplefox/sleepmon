...
def test_compute_production_favorite_berry_doubles_strength(service: DefaultTeamService) -> None:
    plain = service.compute_production(_pikachu())
    favorite = service.compute_production(_pikachu(scenario="favorite"))
...
def test_compute_production_expert_berry_scenario(service: DefaultTeamService) -> None:
    plain = service.compute_production(_pikachu())
    expert = service.compute_production(_pikachu(scenario="expert_berry"))
...
def test_compute_production_expert_ingredient_scenario(service: DefaultTeamService) -> None:
    favorite = service.compute_production(_pikachu(scenario="favorite"))
    expert = service.compute_production(_pikachu(scenario="expert_ingredient"))
...
def test_compute_production_expert_skill_scenario(service: DefaultTeamService) -> None:
    favorite = service.compute_production(_pikachu(scenario="favorite"))
    expert = service.compute_production(_pikachu(scenario="expert_skill"))
    assert expert.skill_triggers > favorite.skill_triggers
    assert expert.night_skill_chances[0] > favorite.night_skill_chances[0]


def test_compute_production_scenario_is_never_the_main_berry(service: DefaultTeamService) -> None:
    # Comparison reads every card as a SUB favorite: no x0.9 cadence, no Skill +1,
    # which belong to the main berry alone.
    plain = service.compute_production(_pikachu(skill_level=3))
    for scenario in ("expert_berry", "expert_ingredient", "expert_skill"):
        expert = service.compute_production(_pikachu(skill_level=3, scenario=scenario))
        assert expert.effective_skill_level == plain.effective_skill_level == 3
        assert expert.seconds_per_help == plain.seconds_per_help


def test_compute_production_rejects_unknown_scenario(service: DefaultTeamService) -> None:
    with pytest.raises(ValidationError):
        service.compute_production(_pikachu(scenario="double_xp"))
