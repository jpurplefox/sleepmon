"""Candies and dream shards needed to level a Pokémon up.

Candy-by-candy simulation over exact game tables: each candy grants EXP by the
current level's band and costs dream shards by that same level; a candy's EXP
...
# EXP to go from level L to L+1, index = L-1 (L=1..69). Source: game8 / RaenonX.
NORMAL_EXP_TO_NEXT: Final[tuple[int, ...]] = (
    ...
    2922, 2977, 3029, 3077, 3095, 3116, 3144, 3189, 3255,
)

# Dream shards per candy spent at level L, index = L-1 (L=1..69). Source: Serebii / RaenonX.
DREAM_SHARDS_PER_CANDY: Final[tuple[int, ...]] = (
    ...
    323, 338, 356, 372, 391, 437, 486, 538, 593, 651,
    698, 750, 804, 866, 932, 1004, 1084, 1173, 1272,
)

MAX_LEVELABLE_LEVEL: Final[int] = len(NORMAL_EXP_TO_NEXT) + 1  # 70
...
# (exp multiplier, shard multiplier) per boost mode.
...
class LevelUpCost:
    """Cost of leveling from `current_level` to `target_level`."""
    ...
    *,
    ...
    if current_level < 1:
        raise ValidationError(
            f"Current level must be at least 1; got {current_level}."
        )
    if target_level > MAX_LEVELABLE_LEVEL:
        raise ValidationError(
            f"Target level cannot exceed {MAX_LEVELABLE_LEVEL}; got {target_level}."
        )
    if current_level >= target_level:
        raise ValidationError(
            "Target level must be greater than current; "
            f"got current={current_level}, target={target_level}."
        )
    ...
    dream_shards = 0
    boosted_candies = 0
    carry = 0  # EXP carried into the next level
