...
2. **Explore — present at least 2 options.** Before writing any document or touching the design system, produce **at least two** visual mockups of the feature. Each mockup is a **self-contained HTML page**: the design-system tokens (CSS variables) inlined, the feature's screens built by composing the design-system components, realistic placeholder content, and **no JS / no functionality** — the user sees how it looks, not how it works. Build each mockup from the **real** components — reuse/inline their actual CSS so what the user approves is what the code already produces, not a free-hand look-alike. If an option introduces a pattern that is **not** an existing component, say so plainly (it's a new pattern, a candidate for `ds-update`) — don't dress it up as an existing one. Render each so the user can actually open and view it — as an **Artifact** (one per option, or one showing the options side by side); fall back to writing the HTML and serving/opening it in a browser where artifacts aren't available. Lead with your recommendation and say how the options differ.
3. **Iterate until the user picks.** The user chooses one option, combines pieces of several, or asks for changes. Revise the mockups and show them again. Stay in this loop until they approve a direction. **Write nothing durable and change nothing in the design system during exploration.** If seeing the design reveals a **product** change — a missing case, a scope that should shift — raise it: it may refine the PRD (via `design`) before you go on. This is why visual runs before `architect`: the look can reshape the product, and that must settle first.
4. **Detect system growth (only after a direction is approved).** If the chosen design needs a component/token the design system does not have, or hits an inconsistency in it, **flag it to the user** — say what's missing and why. Before calling something a variant of an existing component, **check the real code**: a look-alike is a new component, not a variant. The user decides whether to record it; only with their go-ahead, invoke the `ds-update` skill to change the design system. Never edit the design system yourself.
...
Invoke the `architect` skill next — it designs the technical architecture on top of this visual design. Do not invoke any other skill.
